...
## Staff Panel Configuration

Once osTicket was installed and reachable, the staff panel was configured to reflect KCorp's actual department structure rather than left on generic defaults.

- Created five departments matching KCorp: IT, HR, Finance, Sales, Operations
- osTicket's default "Support (Default)" department was kept and renamed to General Support, serving as a general intake/fallback queue rather than being removed
- The unused default "Maintenance" department was deleted, as it had no relevance to this project and no tickets assigned to it
- Created help topics mapped to the planned ticket scenarios: Password Reset, Account Lockout, Shared Drive/File Access, Software Installation, Printer Issue, Network/WiFi Connectivity, and Contractor Access Request, each routed to the appropriate department
- osTicket's default help topics (Feedback, General Inquiry, Report a Problem, Report a Problem/Access Issue) were left in place but unused, since they didn't map cleanly to the IT-support-specific scenarios planned for this project
- Added the admin account as an agent across all departments, since this project only requires one agent to demonstrate a realistic ticket lifecycle rather than a full simulated support team
  ![Departments configured](Screenshots/Setup/09a-departments-configured.png)
  ![Help topics configured](Screenshots/Setup/09b-help-topics-configured.png)
- Created and verified a test ticket end to end to confirm the configuration worked correctly

**Sendmail mailer error on ticket creation**
Creating a ticket produced a system log error: "Unable to email via Sendmail... Sendmail exited with non-zero exit code 127." This is because the VM has no mail transfer agent installed, so osTicket's attempt to send an email notification fails. Ticket creation itself is unaffected, since the mail send is a separate step from saving the ticket. Consistent with the php-imap omission above, live email notifications are intentionally out of scope for this project since tickets are manually simulated and documented rather than driven by a real mail flow. Left as-is rather than installing a local mail relay (e.g. msmtp or Postfix), since it doesn't affect the project's goals.
...
A working, self-hosted osTicket instance reachable through both the staff control panel and the public ticket-submission portal, running on its own VM inside the existing KCorp lab environment, and configured with KCorp's department structure, help topics, and an active agent. Verified working end to end with a test ticket. Ready for ticket scenario simulation against the KCorp employee roster.
